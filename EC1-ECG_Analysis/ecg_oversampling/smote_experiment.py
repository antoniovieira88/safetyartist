...
if __name__ == '__main__':
    ...
    for fold_number, (train_index, test_index) in enumerate(kf.split(x, y)):
        ...
        a, p, r, f = metrics_values(y_pred, y_test, 'holdout_test', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))
        acc.append(a)
        precision.append(p)
        recall.append(r)
        f1.append(f)
        ...
        if np.size(x_cc01_data.size > 0):
            ...
            write_corner_case_csv(x_cc01_safety[:, 0], y_cc01, y_cc01_pred, 'cc01', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))
        
        # b) 1 similar to 0
        if np.size(x_cc10_data.size > 0):
            y_cc10_pred = base_model.predict(x_cc10_data)
            write_corner_case_csv(x_cc10_safety[:, 0], y_cc10, y_cc10_pred, 'cc10', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # c) 0 similar to 2
        if np.size(x_cc02_data.size > 0):
            y_cc02_pred = base_model.predict(x_cc02_data)
            write_corner_case_csv(x_cc02_safety[:, 0], y_cc02, y_cc02_pred, 'cc02', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # d) 2 similar to 0
        if np.size(x_cc20_data.size > 0):
            y_cc20_pred = base_model.predict(x_cc20_data)
            write_corner_case_csv(x_cc20_safety[:, 0], y_cc20, y_cc20_pred, 'cc20', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # e) 0 similar to 3
        if np.size(x_cc03_data.size > 0):
            y_cc03_pred = base_model.predict(x_cc03_data)
            write_corner_case_csv(x_cc03_safety[:, 0], y_cc03, y_cc03_pred, 'cc03', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # f) 3 similar to 0
        if np.size(x_cc30_data.size > 0):
            y_cc30_pred = base_model.predict(x_cc30_data)
            write_corner_case_csv(x_cc30_safety[:, 0], y_cc30, y_cc30_pred, 'cc30', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # g) 0 similar to 4
        if np.size(x_cc04_data.size > 0):
            y_cc04_pred = base_model.predict(x_cc04_data)
            write_corner_case_csv(x_cc04_safety[:, 0], y_cc04, y_cc04_pred, 'cc04', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # h) 4 similar to 0
        if np.size(x_cc40_data.size > 0):
            y_cc40_pred = base_model.predict(x_cc40_data)
            write_corner_case_csv(x_cc40_safety[:, 0], y_cc40, y_cc40_pred, 'cc40', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # i) 1 similar to 2
        if np.size(x_cc12_data.size > 0):
            y_cc12_pred = base_model.predict(x_cc12_data)
            write_corner_case_csv(x_cc12_safety[:, 0], y_cc12, y_cc12_pred, 'cc12', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # j) 2 similar to 1
        if np.size(x_cc21_data.size > 0):
            y_cc21_pred = base_model.predict(x_cc21_data)
            write_corner_case_csv(x_cc21_safety[:, 0], y_cc21, y_cc21_pred, 'cc21', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # k) 1 similar to 3
        if np.size(x_cc13_data.size > 0):
            y_cc13_pred = base_model.predict(x_cc13_data)
            write_corner_case_csv(x_cc13_safety[:, 0], y_cc13, y_cc13_pred, 'cc13', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # l) 3 similar to 1
        if np.size(x_cc31_data.size > 0):
            y_cc31_pred = base_model.predict(x_cc31_data)
            write_corner_case_csv(x_cc31_safety[:, 0], y_cc31, y_cc31_pred, 'cc31', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # m) 1 similar to 4 - empty
        if np.size(x_cc14_data.size > 0):
            y_cc14_pred = base_model.predict(x_cc14_data)
            write_corner_case_csv(x_cc14_safety[:, 0], y_cc14, y_cc14_pred, 'cc14', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # n) 4 similar to 1
        if np.size(x_cc41_data.size > 0):
            y_cc41_pred = base_model.predict(x_cc41_data)
            write_corner_case_csv(x_cc41_safety[:, 0], y_cc41, y_cc41_pred, 'cc41', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # o) 2 similar to 3
        if np.size(x_cc23_data.size > 0):
            y_cc23_pred = base_model.predict(x_cc23_data)
            write_corner_case_csv(x_cc23_safety[:, 0], y_cc23, y_cc23_pred, 'cc23', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # p) 3 similar to 2
        if np.size(x_cc32_data.size > 0):
            y_cc32_pred = base_model.predict(x_cc32_data)
            write_corner_case_csv(x_cc32_safety[:, 0], y_cc32, y_cc32_pred, 'cc32', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # q) 2 similar to 4
        if np.size(x_cc24_data.size > 0):
            y_cc24_pred = base_model.predict(x_cc24_data)
            write_corner_case_csv(x_cc24_safety[:, 0], y_cc24, y_cc24_pred, 'cc24', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # r) 4 similar to 2
        if np.size(x_cc42_data.size > 0):
            y_cc42_pred = base_model.predict(x_cc42_data)
            write_corner_case_csv(x_cc42_safety[:, 0], y_cc42, y_cc42_pred, 'cc42', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # s) 3 similar to 4
        if np.size(x_cc34_data.size > 0):
            y_cc34_pred = base_model.predict(x_cc34_data)
            write_corner_case_csv(x_cc34_safety[:, 0], y_cc34, y_cc34_pred, 'cc34', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))

        # t) 4 similar to 3
        if np.size(x_cc43_data.size > 0):
            y_cc43_pred = base_model.predict(x_cc43_data)
            write_corner_case_csv(x_cc43_safety[:, 0], y_cc43, y_cc43_pred, 'cc43', 'SMOTE_Fold' + str(fold_number + 1).zfill(2))
